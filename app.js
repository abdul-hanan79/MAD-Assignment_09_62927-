...
function stu2(le) {
    ...
    var h = " ";
    for (var i in student2) {
        h = h + student2[i] + " ";
    }
    var len = Object.keys(student2).length;
    console.log(len);
}
// stu2(2);
...
function lib(lib) {
    ...
    for (var i = 0; i < library.length; i++) {
        var b = library[i].title + " written By " + library[i].author;
        if (library[i].readingStatus == true) {
            console.log("Already read : ".concat(b));
        }
        else {
            console.log("Still not read : ".concat(b));
        }
    }
}
// lib('a');
...
var cyln = /** @class */ (function () {
    function cyln(height, diameter) {
        this.height = height;
        this.diameter = diameter;
        // console.log(this.diameter)
    }
    cyln.prototype.cylander = function () {
...
var subs = function () {
    var str = 'dog';
    for (var i = 0; i < 1; i++) {
        var qp = [str.substring(1, 0), str.substring(0, 2), str.substring(0, 3), str.substring(1, 2), str.substring(1, 3), str.substring(2, 3)];
        console.log(qp);
    }
};
// subs()
...
var circle = /** @class */ (function () {
    ...
    circle.prototype.area = function () {
        var r = 2 * Math.PI * this.radius * this.radius;
        console.log(r);
    };
    circle.prototype.perameter = function () {
        var pera = 2 * Math.PI * this.radius;
        console.log(pera);
    };
    return circle;
}());
...
var now = function () {
    var url = new URL('https://www.fiverr.com/mkiachannel?up_rollout=true');
    console.log(url);
};
// now()
...
var fun = function () {
    ...
    var ke = list.hasOwnProperty("fName");
    console.log(ke);
};
// fun()
